...
{ 
  ...
  query_obj = new pqxx::nontransaction(*conn);
...
vector<attribute> MyFIUPgParser::parseAttributeList()
{
  ...
  std::string sql_query = "SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = '" + course_table_name + "';"; 
  pqxx::result num_of_col_query_result (query_obj->exec(sql_query));
  std::string buffer = num_of_col_query_result.begin()[0].as<std::string>();
  
  num_of_columns = std::stoi(buffer);
  
  /* Get all column names */
  sql_query = "SELECT column_name FROM INFORMATION_SCHEMA.COLUMNS WHERE table_name = '" + course_table_name + "';"; 
...
vector<vector<string>> MyFIUPgParser::parseRecords()
{
  ...
  std::string sql_query = "SELECT * FROM " + course_table_name + ";"; 
